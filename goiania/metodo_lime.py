...
def Lime(X,y):
    ...
    expl = lime.lime_tabular.LimeTabularExplainer(treinamento_x, feature_names=['Dormitorios', 'Garagens', 'Banheiros', 'Tipo', 'Bairro', 'Cidade','Padrao'],class_names=['Baixo', 'Medio', 'Alto'])

    prever = lambda x: classificador_forest.predict_proba(x).astype(float)

    print(validacao_x[2])
    print(prever(validacao_x[2].reshape(1,-1)))
    print(classificador_forest.predict(validacao_x[2].reshape(1,-1)))
