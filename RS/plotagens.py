...
def plotBairro_TD(df):
    ...
    df_grouped = df.groupby(['Bairro', 'Tipo','QtdDormitorio']).size().reset_index(name='contagem')
    df_grouped['contagem'] = pd.to_numeric(df_grouped['contagem'], errors='coerce')
    ...
    plt.figure(figsize=(10,5))
    plt.barh(df_top10['Bairro'] + ' - ' + df_top10['Tipo'] + ' - ' + df_top10['QtdDormitorio'].astype(str) + ' quartos', df_top10['contagem'])
    plt.xlabel('Contagem')
...
def plot_TDB(df):
    ...
    df_grouped = df.groupby(['Tipo','QtdDormitorio','QtdBanheiro']).size().reset_index(name='contagem')
    df_grouped['contagem'] = pd.to_numeric(df_grouped['contagem'], errors='coerce')
    ...
    plt.figure(figsize=(10,5))
    plt.barh(df_top10['Tipo'] + ' - ' + df_top10['QtdDormitorio'].astype(str) + ' quartos' + df_top10['QtdBanheiro'].astype(str) + ' banheiros', df_top10['contagem'])
    plt.xlabel('Contagem')
    plt.ylabel('Bairro - Tipo de Imóvel - Quartos')
    plt.title('Os 10 tipos de imóveis mais comuns por bairro e número de quartos')
    plt.show()